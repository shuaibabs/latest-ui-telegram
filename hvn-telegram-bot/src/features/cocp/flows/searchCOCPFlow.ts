...
export async function startSearchCOCPFlow(bot: TelegramBot, chatId: number, username?: string) {
    ...
    setSession(chatId, 'searchCOCP', {
        stage: 'SELECT_TYPE',
        criteria: {}
    });

    await bot.sendMessage(chatId, "🔍 *Search COCP Records*\n\nChoose search type:", {
        parse_mode: 'Markdown',
        reply_markup: {
            inline_keyboard: [
...
export function registerSearchCOCPFlow(router: CommandRouter) {
    const bot = router.bot;

    bot.on('message', async (msg: TelegramBot.Message) => {
        const session = getSession(msg.chat.id, 'searchCOCP') as SearchSession | undefined;
        ...
        if (session.stage === 'AWAIT_CRITERIA_VAL' && session.currentSetting) {
            ...
            clearSession(chatId, 'searchCOCP');
        }
    });

    router.registerCallback(/^cocp_search_type_/, async (query) => {
        const chatId = query.message!.chat.id;
        const session = getSession(chatId, 'searchCOCP') as SearchSession | undefined;
        if (!session) return;
        ...
        if (type === 'adv') {
            ...
            setSession(chatId, 'searchCOCP', session);
            await bot.sendMessage(chatId, "*Advanced COCP Search*\nConfigure your filters:", {
                parse_mode: 'Markdown',
                reply_markup: getCriteriaMenu(session.criteria)
            });
        } else {
    ...
    router.registerCallback(/^cocp_search_set_/, async (query) => {
        const chatId = query.message!.chat.id;
        const session = getSession(chatId, 'searchCOCP') as SearchSession | undefined;
        if (!session || session.stage !== 'ADV_SEARCH_MENU') return;

        const key = query.data?.replace('cocp_search_set_', '');
    ...
    router.registerCallback('cocp_search_apply', async (query) => {
        const chatId = query.message!.chat.id;
        const session = getSession(chatId, 'searchCOCP') as SearchSession | undefined;
        if (!session) return;
    ...
    router.registerCallback('cocp_search_cancel', async (query) => {
        clearSession(query.message!.chat.id, 'searchCOCP');
...
async function performSearch(bot: TelegramBot, chatId: number, criteria: any, username?: string) {
    try {
        ...
        const results = await searchCOCPNumbers(criteria, employeeName);
        if (results.length === 0) {
            await bot.sendMessage(chatId, "🔍 No COCP numbers found matching your criteria.");
        } else {
        ...
        logger.error(`Error in performSearchCOCP: ${error.message}`);
