...
exports.startSearchCOCPFlow = startSearchCOCPFlow;
exports.registerSearchCOCPFlow = registerSearchCOCPFlow;
...
const cancelBtn = { text: '❌ Cancel', callback_data: 'cocp_search_cancel' };
...
function startSearchCOCPFlow(bot, chatId, username) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        (0, sessionManager_1.setSession)(chatId, 'searchCOCP', {
            stage: 'SELECT_TYPE',
            criteria: {}
        });
        yield bot.sendMessage(chatId, "🔍 *Search COCP Records*\n\nChoose search type:", {
            parse_mode: 'Markdown',
            reply_markup: {
                inline_keyboard: [
                    [{ text: '🔍 Advanced Search', callback_data: 'cocp_search_type_adv' }],
                    [{ text: '🔢 Must Contains (Digits Only)', callback_data: 'cocp_search_type_must' }],
                    [cancelBtn]
                ]
            }
        });
...
function getCriteriaMenu(criteria) {
    ...
    for (const key of keys) {
        ...
        rows.push([{ text: `${criteriaLabels[key]}: ${val}`, callback_data: `cocp_search_set_${key}` }]);
    }
    rows.push([{ text: '✅ Apply Search', callback_data: 'cocp_search_apply' }]);
    rows.push([cancelBtn]);
    return { inline_keyboard: rows };
}
function registerSearchCOCPFlow(router) {
    const bot = router.bot;
    bot.on('message', (msg) => __awaiter(this, void 0, void 0, function* () {
        var _a;
        const session = (0, sessionManager_1.getSession)(msg.chat.id, 'searchCOCP');
        ...
        if (session.stage === 'AWAIT_CRITERIA_VAL' && session.currentSetting) {
            ...
            (0, sessionManager_1.setSession)(chatId, 'searchCOCP', session);
            ...
            (0, sessionManager_1.clearSession)(chatId, 'searchCOCP');
        }
    }));
    router.registerCallback(/^cocp_search_type_/, (query) => __awaiter(this, void 0, void 0, function* () {
        var _a;
        const chatId = query.message.chat.id;
        const session = (0, sessionManager_1.getSession)(chatId, 'searchCOCP');
        if (!session)
            return;
        ...
        if (type === 'adv') {
            ...
            (0, sessionManager_1.setSession)(chatId, 'searchCOCP', session);
            yield bot.sendMessage(chatId, "*Advanced COCP Search*\nConfigure your filters:", {
                parse_mode: 'Markdown',
                reply_markup: getCriteriaMenu(session.criteria)
            });
        }
    ...
    router.registerCallback(/^cocp_search_set_/, (query) => __awaiter(this, void 0, void 0, function* () {
        var _a;
        const chatId = query.message.chat.id;
        const session = (0, sessionManager_1.getSession)(chatId, 'searchCOCP');
        if (!session || session.stage !== 'ADV_SEARCH_MENU')
            return;
        const key = (_a = query.data) === null || _a === void 0 ? void 0 : _a.replace('cocp_search_set_', '');
    ...
    router.registerCallback('cocp_search_apply', (query) => __awaiter(this, void 0, void 0, function* () {
        const chatId = query.message.chat.id;
        const session = (0, sessionManager_1.getSession)(chatId, 'searchCOCP');
        if (!session)
            return;
    ...
    router.registerCallback('cocp_search_cancel', (query) => __awaiter(this, void 0, void 0, function* () {
        (0, sessionManager_1.clearSession)(query.message.chat.id, 'searchCOCP');
...
function performSearch(bot, chatId, criteria, username) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const results = yield (0, cocpService_1.searchCOCPNumbers)(criteria, employeeName);
            if (results.length === 0) {
                yield bot.sendMessage(chatId, "🔍 No COCP numbers found matching your criteria.");
            }
            else {
                const count = results.length;
                let text = `🔍 *COCP Search Results (${count})*\n`;
                text += `━━━━━━━━━━━━━━━━━━━━\n\n`;
            ...
            logger_1.logger.error(`Error in performSearchCOCP: ${error.message}`);
